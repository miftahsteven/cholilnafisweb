...
async function mediaRoutes(fastify) {
    ...
    fastify.post('/register', {
        preHandler: [auth_middleware_1.authMiddleware],
        handler: async (request, reply) => {
            let { filename, url, type, size, mimeType, alt } = request.body;
                ...
                if (ytMatch && ytMatch[1]) {
                    ...
                    if (fetchedBuffer) {
                        try {
                            ...
                            const ext = 'jpg';
                            const thumbFilename = `yt-${videoId}-${crypto_1.default.randomUUID()}.${ext}`;
                            const uploadsDir = path_1.default.join(__dirname, '../../public/uploads');
                            if (!fs_1.default.existsSync(uploadsDir)) {
                                fs_1.default.mkdirSync(uploadsDir, { recursive: true });
                            }
                            const filepath = path_1.default.join(uploadsDir, thumbFilename);
                            fs_1.default.writeFileSync(filepath, buffer);
                            const host = request.headers.host || process.env.BACKEND_URL?.replace(/^https?:\/\//, '') || 'localhost:4000';
