...
class KiAiController {
    async calculateUsedQuota(userId) {
        return ki_ai_service_1.kiAiService.calculateUsedQuota(userId);
    }
    async handleChat(request, reply) {
        try {
            ...
            if (final_user_id) {
                const isBlocked = await prisma_1.prisma.blockedKiAiUser.findUnique({ where: { userId: final_user_id } });
                if (isBlocked) {
    ...
    async getSessions(request, reply) {
        try {
            ...
            const dailyCount = await this.calculateUsedQuota(final_user_id);
            const logs = await prisma_1.prisma.chatLog.findMany({
                where: { userId: final_user_id },
            ...
            const isBlocked = await prisma_1.prisma.blockedKiAiUser.findUnique({
                where: { userId: final_user_id }
            });
    ...
    async getSessionDetails(request, reply) {
        try {
            ...
            const messages = await prisma_1.prisma.chatLog.findMany({
                where: { sessionId },
                include: {
                    sources: true,
                    feedbacks: true
    ...
    async provideFeedback(request, reply) {
        try {
            const { chatId, isHelpful } = request.body;
            if (!chatId)
                return reply.status(400).send({ error: 'chatId is required' });
            await prisma_1.prisma.feedback.upsert({
                where: { chatId },
                update: { isHelpful },
    ...
    async getAllQuestions(request, reply) {
        try {
            const logs = await prisma_1.prisma.chatLog.findMany({
                orderBy: { createdAt: 'desc' },
                include: { feedbacks: true }
            });
            const blockedUsers = await prisma_1.prisma.blockedKiAiUser.findMany();
    ...
    async deleteQuestion(request, reply) {
        try {
            ...
            await prisma_1.prisma.chatLog.delete({ where: { id } });
    ...
    async resetUserQuestions(request, reply) {
        try {
            const { userId } = request.params;
            const startOfToday = new Date();
            startOfToday.setHours(0, 0, 0, 0);
            const deleted = await prisma_1.prisma.chatLog.deleteMany({
                where: {
                    userId,
    ...
    async wipeUserQuestions(request, reply) {
        try {
            const { userId } = request.params;
            await prisma_1.prisma.chatLog.deleteMany({ where: { userId } });
    ...
    async unblockUser(request, reply) {
        try {
            const { userId } = request.params;
            await prisma_1.prisma.blockedKiAiUser.delete({ where: { userId } });
...
exports.KiAiController = KiAiController;
exports.kiAiController = new KiAiController();
