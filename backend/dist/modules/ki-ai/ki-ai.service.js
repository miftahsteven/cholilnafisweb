...
exports.kiAiService = exports.KiAiService = void 0;
...
class KiAiService {
    async calculateUsedQuota(userId) {
        const startOfToday = new Date();
        startOfToday.setHours(0, 0, 0, 0);
        const logsToday = await prisma_1.prisma.chatLog.findMany({
            where: {
                userId,
    ...
    async askFullAnswer(message, sessionId, userId, userName, userEmail) {
        // 0. Initial Log
        ...
        try {
            ...
            if (userId) {
                const isBlocked = await prisma_1.prisma.blockedKiAiUser.findUnique({ where: { userId } });
                if (isBlocked) {
                    ...
                    await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { answer: blockMsg, mode: 'blocked' } });
                    return blockMsg;
                }
            }
            // 2. Moderation
            const category = await moderation_engine_1.moderationEngine.classifyMessage(message);
            if (category === 'BAD') {
                ...
                await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { answer: blockText, mode: 'blocked' } });
                if (userId) {
                    await prisma_1.prisma.blockedKiAiUser.upsert({
                        where: { userId },
            ...
            if (category === 'OFF_TOPIC') {
                ...
                await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { answer: friendlyMsg, mode: 'off-topic', confidence: 1.0 } });
                return friendlyMsg;
            }
            if (category === 'GREETING') {
                ...
                await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { answer: friendlyMsg, mode: 'greeting', confidence: 1.0 } });
                return friendlyMsg;
            }
            // 3. Rate Limit
            if (userId) {
                ...
                if (dailyCount >= limit) {
                    ...
                    await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { answer: limitMsg, mode: 'rate-limited' } });
                    return limitMsg;
                }
            }
            // 4. Search & Decide
            ...
            await prisma_1.prisma.chatLog.update({ where: { id: initialLog.id }, data: { mode, confidence } });
            if (sources.length > 0) {
            ...
            const fullAnswer = await llm_engine_1.llmEngine.generate(message, mode, internalResults, externalResults, includeDalil);
...
exports.KiAiService = KiAiService;
exports.kiAiService = new KiAiService();
