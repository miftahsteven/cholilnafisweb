...
exports.llmEngine = exports.LLMEngine = void 0;
const openai_1 = __importDefault(require("openai"));
const openai = new openai_1.default({
    apiKey: process.env.OPENAI_API_KEY || '',
});
class LLMEngine {
    basePrompt = `Anda adalah KI.AI, asisten keislaman cerdas yang merepresentasikan pemikiran K.H. Cholil Nafis dan selaras dengan Majelis Ulama Indonesia (MUI).
...
`;
    buildPrompt(question, mode, internalData, externalData, includeDalil = false) {
        let contextText = '';
...
`;
    ...
    async buildAndStreamPrompt(question, mode, internalData, externalData, includeDalil = false) {
    ...
    async generate(question, mode, internalData, externalData, includeDalil = false) {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error('OPENAI_API_KEY is not configured');
        }
        const finalPrompt = this.buildPrompt(question, mode, internalData, externalData, includeDalil);
...
exports.llmEngine = new LLMEngine();
